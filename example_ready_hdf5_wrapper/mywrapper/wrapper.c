...
//gotcha handler for shared wrapper
...
//Function prototypes are needed to be declared for static wrapper
ssize_t __real_H5Fcreate(const char * name, unsigned flags, hid_t fcpl_id, hid_t fapl_id);

ssize_t __real_H5Fopen( const char *name, unsigned flags, hid_t fapl_id );

ssize_t __real_H5Fclose( hid_t file_id );
...
//Binding shared wrapper with gotcha
